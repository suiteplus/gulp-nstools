...
module.exports = through.obj(function (chunk, enc, callback) {
    let bundleCfgPath = chunk.path.substr(chunk.cwd.length + 1),
        bundleCfgDir = bundleCfgPath.replace('/bundle.json', ''),
    ...
    let actual = 0,
        verifyNext = () => {
            if (++actual === bundleScripts.length) callback(null, chunk);
        };
    ...
    for (let i = 0; i < bundleScripts.length; i++) {
        ...
        for (let f = 0; f < files.length; f++) {
            ...
            nsify(fileJs)
                .pipe(plugins.uglify({
                    output: {
                        screw_ie8: config.clientProof === true,
                        beautify: ~process.argv.indexOf('--no-compress')
                    },
                    compress: {
                        drop_console: true
                    }
